...
function Calendar({
  ...
  yearRange = 50,
  ...props
}: DayPickerSingleProps & { yearRange?: number }) {
  ...
  const CustomCaption = ({ month }: { month: Date }) => (
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        nav: 'space-x-1 flex items-center absolute gap-[5.5rem] justify-between',
        ...
        head_cell: cn(
          ...
          props.showWeekNumber && 'last:hidden'
        ),
        row: 'flex w-full mt-2',
        cell: cn(
          'h-9 w-9 text-center text-sm p-0 relative [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20',
          props.showWeekNumber && 'first:hidden'
        ),
        day: cn(
...
} & Pick<DayPickerSingleProps, 'locale' | 'weekStartsOn' | 'showWeekNumber' | 'showOutsideDays'>;
